...
public class VolumetricLight : MonoBehaviour
{
  ...
  private void OnEnable() => VolumetricLightRenderer.PreRenderEvent += new Action<VolumetricLightRenderer, Matrix4x4>(this.VolumetricLightRenderer_PreRenderEvent);

  private void OnDisable() => VolumetricLightRenderer.PreRenderEvent -= new Action<VolumetricLightRenderer, Matrix4x4>(this.VolumetricLightRenderer_PreRenderEvent);
  ...
  private bool IsCameraInSpotLightBounds() => (double) Vector3.Dot(this._light.transform.forward, Camera.current.transform.position - this._light.transform.position) <= (double) (this._light.range + 1f) && (double) Mathf.Acos(Vector3.Dot(this.transform.forward, (Camera.current.transform.position - this._light.transform.position).normalized)) * 57.2957801818848 <= ((double) this._light.spotAngle + 3.0) * 0.5;
}
